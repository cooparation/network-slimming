...
def convert_locations_to_boxes(locations, priors, center_variance,
    ...
    if 'cuda' in str(locations[..., :2].device):
        return torch.cat([
            ...
            torch.exp(locations[..., 2:] * size_variance) * priors[..., 2:].cuda()
        ], dim=locations.dim() - 1)
    else:
        return torch.cat([
            locations[..., :2] * center_variance * priors[..., 2:].cpu() + priors[..., :2].cpu(),
            torch.exp(locations[..., 2:] * size_variance) * priors[..., 2:].cpu()
        ], dim=locations.dim() - 1)

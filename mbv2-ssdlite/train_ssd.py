...
# do the sparsity regularization
...
def updateBN(model):
    for name, m in model.named_modules():
        if 'basenet' in name and isinstance(m, nn.BatchNorm2d):
            if 'conv/bn' in name: # first layer keep less sprsity
                m.weight.grad.data.add_(args.s*0.5*torch.sign(m.weight.data))  # L1
            else:
...
def get_learning_rate(optimizer):
    ...
    #lr_type=['lr', 'base_net_lr', 'extra_layers_lr']
    lr_type= 'lr'
    for param_group in optimizer.param_groups:
          lr +=[ param_group[lr_type] ]
    return lr
...
if __name__ == '__main__':
    ...
    if args.net == 'vgg16-ssd':
        ...
        invert_cfg = [# hidden_c, out_c, s, expand_ratio
                ...
                [960, 160, 2, -1],
                [960, 320, 1, -1],]
        create_net = lambda num: create_mobilenetv2_ssd_lite_prune(num,
